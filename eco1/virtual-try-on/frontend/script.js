...
async function init() {
    ...
    if (productImage) {
        ...
        const product = {
            ...
            name: productName || 'Imported Item',
            image: decodeURIComponent(productImage)
        };

        // Add ONLY this product to the sidebar
        addImportedProductToSidebar(product);
    } else {
        // No product specified, load all from backend
        await loadProducts();
    }
}

function addImportedProductToSidebar(product) {
    const div = document.createElement('div');
    div.className = 'product-card';
    div.onclick = () => addProductFromUrl(product);
    ...
    if (product.image.startsWith('http') || product.image.startsWith('/')) {
        ...
        // Relative path from PHP (e.g. 'uploads/img.jpg') -> needs '../../e/' prefix
        img.src = '../../e/' + product.image;
    }

    // Update product image path for when it's used later
...
function addProductFromUrl(product) {
    const img = new Image();
    img.crossOrigin = "Anonymous";
    img.onload = () => {
        const newOverlay = {
    ...
    img.src = product.image; // Already resolved in sidebar
}

async function loadProducts() {
    try {
        // Fetch from PHP backend instead of Python API
        ...
        products.forEach(product => {
            ...
            if (product.category && product.category !== lastCategory) {
            ...
            const img = document.createElement('img');
            // Image path is already processed by PHP backend to be relative to this file
            img.src = product.image;
            img.crossOrigin = "Anonymous";
